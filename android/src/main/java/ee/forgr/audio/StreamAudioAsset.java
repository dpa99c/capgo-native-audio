...
public class StreamAudioAsset extends AudioAsset {
    ...
    private void fadeIn(double fadeInDurationMs, float volume) {
        ...
        fadeTask = fadeExecutor.scheduleWithFixedDelay(new Runnable() {
            ...
            public void run() {
                ...
                Log.d(TAG, "Fade in step: from " + previousCurrentVolume + " to " + currentVolume + " to target " + resolvedTargetVolume);
    ...
    private void fadeTo(double fadeDurationMs, float targetVolume) {
        ...
        fadeTask = fadeExecutor.scheduleWithFixedDelay(new Runnable() {
            ...
            @Override
            public void run() {
                if (fadeState != FadeState.FADE_TO || !player.isPlaying()) {
                    fadeState = FadeState.NONE;
                    cancelFade();
                ...
                Log.d(TAG, "Fade to step: from " + previousCurrentVolume + " to " + currentVolume + " to target " + resolvedTargetVolume);
                player.setVolume(resolvedTargetVolume);
            }
        }, 0, FADE_DELAY_MS, TimeUnit.MILLISECONDS);
    }
