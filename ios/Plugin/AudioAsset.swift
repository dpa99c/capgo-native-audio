...
public class AudioAsset: NSObject, AVAudioPlayerDelegate {
    ...
    init(owner: NativeAudio, withAssetId assetId: String, withPath path: String!, withChannels channels: Int!, withVolume volume: Float!) {
        ...
        self.fadeQueue = DispatchQueue(label: "com.audioasset.fadeQueue")
     ...
     */
    func play(time: TimeInterval, volume: Float??) {
        ...
        owner?.executeOnAudioQueue { [weak self] in
            ...
            player.volume = (volume ?? self.initialVolume) ?? Constant.DefaultVolume
            player.play()
    ...
    */
    ...
    func fadeTo(audio: AVAudioPlayer, fadeOutDuration: TimeInterval, targetVolume: Float) {
        cancelFade() // Cancel any ongoing fade
        let steps = Int(fadeOutDuration / TimeInterval(FADE_DELAY))
        guard steps > 0 else { return }
        var currentVolume: Float = audio.volume
        ...
        owner?.executeOnAudioQueue { [weak self] in
            ...
            for player in channels {
                if(player.isPlaying && fadeDuration > 0){
                    ...
                    self.fadeTo(audio: player, fadeOutDuration: fadeDuration, targetVolume: validVolume)
                }else{
